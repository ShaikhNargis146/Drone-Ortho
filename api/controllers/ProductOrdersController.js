	...
	paymentNotify: function (req, res) {
		...
		if (req.body.payload.id) {
			...
			ctrl.execute(function () {
				...
				if (response != null && paymentRes.transaction.recurringBilling == false) {
					if (response.getMessages().getResultCode() == ApiContracts.MessageTypeEnum.OK) {
				...
				} else if (response != null && paymentRes.transaction.recurringBilling == true) {
					if (response.getMessages().getResultCode() == ApiContracts.MessageTypeEnum.OK) {
						async.waterfall([
							function (callback) {
								ProductOrders.findOne({
			...
			ProductOrders.findOne({
				...
				if (err || _.isEmpty(data)) {
					...
					if (req.query.paymentType == "paypal") {
						...
						var settingValue = {
							'showReceipt': false,
							'url': global["env"].realHost + '/api/ProductOrders/paymentReturn?invoiceNumber=' + req.query.invoiceNumber,
							'urlText': 'Continue',
							'cancelUrl': global["env"].realHost + '/api/ProductOrders/paymentCancel',
